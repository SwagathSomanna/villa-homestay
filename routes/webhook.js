...
router.post('/cashfree', express.json({ verify: rawBodySaver }), async (req, res) => {
  try {
    ...
    if (type === 'PAYMENT_SUCCESS') {
      ...
      const booking = state.bookings.find(b => b.cashfreeOrderId === order_id || b.cf_order_id === order_id);
      if (!booking) {
      ...
      console.log(`[WEBHOOK] Room blocked for dates: ${booking.checkInDate} to ${booking.checkOutDate}`);
      ...
      if (booking) {
        ...
        await writeState(state);
        console.log(`[WEBHOOK] PAYMENT_FAILED - Auto-cancelled booking ${booking.id}: ${payment_message}`);
      }
      ...
      const booking = state.bookings.find(b => b.cashfreePaymentId === cf_payment_id);
      if (!booking) {
        console.log(`[WEBHOOK] Booking not found for payment ${cf_payment_id}`);
